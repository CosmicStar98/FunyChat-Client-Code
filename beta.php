...
<div class=chatdiv id=chatdiv style="display: none"><div class=testeg2 id=testeg2><div id=testeg class=testeg ></div></div><form action="javascript:send()"><input class=msgin autocomplete=off id=sendeg> <input type=submit class=msgbtn value=send></form>
<div class=stats>
...
<p id=usercount>User Count: getting...</p><br><div id=namelist><h3>user list</h3></div></div></div></div><div id=namediv><div id=nameform><form action="javascript:showeverything()"><h1 style="font-size:35px;"> Join a room </h1><h3> nickname </h3><input id=nameput required><br><h3> Room (optional)</h3> <input id=room> <br><br><input type=submit id=namesub value="join"></form></div>
</div>
<script>
var ws;
var kickedeg = 0;
...
ws = new WebSocket("wss://chat-with-rooms.colinkrzemienow.repl.co");
ws.onopen = ()=>{
    if (document.getElementById("room").value == ''){
document.getElementById("room5").innerHTML = 'Room: main';
ws.send('room:main');
    }else{
document.getElementById("room5").innerHTML = 'Room: '+ document.getElementById("room").value
